...
pnpm run dev:react-kit      # packages/react-kit (tsc --watch)
pnpm run dev:ts-kit         # packages/ts-kit (tsc --watch)
pnpm run dev:ts-types       # packages/ts-types (tsc --watch)
...
`@forthtilliath/shadcn-ui` n'a pas de script
